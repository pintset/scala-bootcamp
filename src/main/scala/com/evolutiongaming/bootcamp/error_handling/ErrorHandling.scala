...
object ErrorHandling extends App {
  ...
  def parseIntEither(string: String): Either[String, Int] =
    //string.toIntOption.map(Right(_)).getOrElse(Left(s"$string does not contain an integer"))
  ...
  def credit(amount: BigDecimal): Either[TransferError, Unit] = {
    ...
    else if (amount == 0) Left(TransferError.NegativeAmount)
    else if (amount >= 1000000) Left(TransferError.ZeroAmount)
  ...
  object StudentValidator {
    ...
    private def validateAge(age: String): AllErrorsOr[Int] = {
      val ageInt = age.toIntOption

      def validateAgeNumericRepresentation: AllErrorsOr[String] =
        ageInt.map { _ => age.validNec }.getOrElse(AgeIsNotNumeric.invalidNec)

      def validateAgeBounds: AllErrorsOr[String] = ageInt match {
        case Some(a) if a >= 18 && a <= 75 => age.validNec
        case _ => AgeIsOutOfBounds.invalidNec
      }

      validateAgeNumericRepresentation.productR(validateAgeBounds)
    }
